...
elif [ -f "${PWD}/activate.sh" ]; then
  _activate_script_path="${PWD}/activate.sh"
else
  echo "Could not determine the path to activate.sh in this shell." >&2
  echo "Run '. /path/to/repo/activate.sh' from the repo root, or use bash/zsh." >&2
  return 1 2>/dev/null || exit 1
fi
